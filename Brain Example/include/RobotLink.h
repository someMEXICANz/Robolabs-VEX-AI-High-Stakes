...
#ifndef ROBOT_LINK_H_
#define ROBOT_LINK_H_
...
#define POS_GPS_CONNECTED   0x01
  ...
  class RobotLink : public vex::serial_link {
      public:
        // robot link is only wireless
        RobotLink( int32_t index, const char *name, vex::linkType type );
        // we never tear down a RobotLink
        ~RobotLink();
        ...
        void set_remote_location( double x, double y, double heading, int32_t status );
        void get_local_location( double &x, double &y, double &heading, int32_t &status );
        void get_remote_location( double &x, double &y, double &heading );
